...
function renderOrders(orders) {
  ...
  content.innerHTML = `
    ...
    <div class="grid grid-3 mb-6">
      <div class="stat-card"><p class="stat-card-value">${total}</p><p class="stat-card-label">Total de Pedidos</p></div>
      <div class="stat-card"><p class="stat-card-value">${active}</p><p class="stat-card-label">Em Andamento</p></div>
      <div class="stat-card"><p class="stat-card-value">${delivered}</p><p class="stat-card-label">Entregues</p></div>
    </div>
    ...
    ${orders.length === 0 ? `<div class="card empty-state"><div class="empty-state-icon" style="font-size:20px">-</div><h3 class="empty-state-title">Nenhum pedido ainda</h3><p class="empty-state-text">Seus pedidos aparecerão aqui.</p></div>` :
    orders.map(order => `
